...
def resolve_tmdb_movie_id(tmdb_id: str | int) -> str | None:
    """Resolve a TMDB movie ID from the anime mapping.
    
    Used to determine if a TMDB anime ID is a movie vs TV series.
    Returns the tmdb_movie_id if found, None otherwise.
    """
    normalized = str(tmdb_id)
    for entry in load_mapping_data().values():
        # Check if this entry has tmdb_movie_id matching the query
        movie_id = entry.get("tmdb_movie_id")
        if movie_id is not None and str(movie_id) == normalized:
            return normalized
        # Also check tmdb_show_id/tmdb_id/tmdb_tv_id - if match but no movie_id,
        # this is a TV series
        for key in ("tmdb_show_id", "tmdb_id", "tmdb_tv_id"):
            if entry.get(key) not in (None, ""):
                if str(entry[key]) == normalized:
                    # Found in mapping as TV, not a movie
                    return None
    return None


def is_tmdb_anime_movie(tmdb_id: str | int) -> bool | None:
    """Check if a TMDB ID is known to be a movie in the anime mapping.
    
    Returns:
        True if the ID is found as tmdb_movie_id (known movie)
        False if the ID is found as tmdb_show_id/tmdb_id/tmdb_tv_id (known TV)
        None if not found in mapping (unknown)
    """
    normalized = str(tmdb_id)
    for entry in load_mapping_data().values():
        movie_id = entry.get("tmdb_movie_id")
